...
const MachineStatus = () => {
  ...
  const filterOrderDetail = (fullOrder: IOrder | undefined) => {
    if (!fullOrder) return;
    ...
    setOrderDetails(washDryOrderDetails);
  };
    ...
    <View className="bg-background-1 h-full">
      <SafeAreaView className="h-full">
        ...
        <View className="mx-8 gap-4">
         { orderDetails?.length > 0 ? orderDetails?.map((detail, index) => (
            <View
              key={index}
          ...
          )) : <Text className="my-10 font-kanit text-text-4 text-2xl align-middle text-center">ยังไม่มีผ้าที่กำลังซักในขณะนี้</Text>}
        </View>
      </SafeAreaView>
    </View>
  );
